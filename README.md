...
This repository demonstrates how [Devin](https://devin.ai) can accelerate SAS-to-PySpark migration for financial services organizations. Using a real-world home equity loan dataset, we show six complete migration examples — from basic data loading to logistic regression modeling and a monthly cohort / vintage report — with side-by-side SAS and PySpark code, inline comments mapping each SAS construct to its PySpark equivalent, and validation tests to ensure correctness.
...
| 6 | `sas/06_vintage_analysis.sas` | `pyspark/06_vintage_analysis.py` | Cohort / vintage report: BY-group `RETAIN` running total with `FIRST.`/`LAST.` flags, `calculated` PROC SQL summary, `%DO` macro threshold sweep stacked with `SET` |
...
python pyspark/06_vintage_analysis.py
```
...
python -m unittest tests.test_06_vintage_analysis -v
```
...
│   └── 06_vintage_analysis.sas                # BY-group RETAIN/FIRST./LAST., PROC SQL calculated, %DO macro sweep
├── pyspark/
...
│   └── 06_vintage_analysis.py                 # Window functions (running sum, row_number), unionByName over a loop
├── migration_guide/
...
└── tests/
    ├── test_pyspark_outputs.py                # Validation tests for scripts 01-05
    └── test_06_vintage_analysis.py            # Parity tests for 06 against a pure-Python replay of the SAS logic
```
