...
This repository demonstrates how [Devin](https://devin.ai) can accelerate SAS-to-PySpark migration for financial services organizations. Using a real-world home equity loan dataset, it provides two worked migration examples (data loading and data cleaning) — with side-by-side SAS and PySpark code and inline comments mapping each SAS construct to its PySpark equivalent — alongside the original SAS programs that are ready to be migrated, plus a validation suite to check correctness.
...
│   └── 02_data_cleaning.py                    # withColumn/when, isNull flags, filter, describe
├── migration_guide/
